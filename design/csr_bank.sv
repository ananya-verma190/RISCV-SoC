    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (trap_en) begin
                ...
                // DEBUG: if a CSR test is hanging (e.g. csrrc-01), this line
                // firing repeatedly at the SAME trap_pc means you're in a
                // trap loop -- almost always caused by mtvec pointing
                // somewhere that itself re-traps (uninitialized mtvec=0,
                // illegal_instr misfire, etc).
                //$display("[CSRBANK][TRAP] trap_pc=%08x -> mepc=%08x mcause=%08x mtval=%08x  (next fetch should go to mtvec)",
                          //trap_pc, trap_pc & 32'hFFFF_FFFC, trap_cause, trap_val);
