...
int main(int argc, char** argv) {
    ...
    for (; cycle < max_cycles; cycle++) {
        dut->clk = 0; dut->eval();   // combinational settle — this IS the committing instruction

        uint32_t pc_now = dut->rootp->Advaita_V__DOT__pc_address;
        uint32_t instr  = dut->rootp->Advaita_V__DOT__instruction;

        fprintf(stderr, "[%llu] [M]: 0x%08x (0x%08x) DASM(0x%08x)\n",
                (unsigned long long)cycle, pc_now, instr, instr);

        bool     reg_we = dut->rootp->Advaita_V__DOT__i_cpu_core__DOT__id_st__DOT__gated_reg_write;
        uint8_t  rd     = dut->rootp->Advaita_V__DOT__i_cpu_core__DOT__id_st__DOT__rd;
        uint32_t wb     = dut->rootp->Advaita_V__DOT__i_cpu_core__DOT__wb_data;
        if (reg_we && rd != 0) {
            fprintf(stderr, "x%u <- 0x%08x\n", rd, wb);
        }

        uint32_t mem_addr = dut->rootp->Advaita_V__DOT__alu_result;
        if (dut->rootp->Advaita_V__DOT__mem_read_en) {
            fprintf(stderr, "mem[X,0x%08x] -> 0x%08x\n", mem_addr,
                    dut->rootp->Advaita_V__DOT__cpu_mem_out);
        }
        if (dut->rootp->Advaita_V__DOT__mem_write_en) {
            fprintf(stderr, "mem[W,0x%08x] <- 0x%08x\n", mem_addr,
                    dut->rootp->Advaita_V__DOT__rs2_data);
        }

        if (dut->rootp->Advaita_V__DOT__csr_write_en) {
            fprintf(stderr, "CSR 0x%03x <- 0x%08x\n",
                    dut->rootp->Advaita_V__DOT__csr_address,
                    dut->rootp->Advaita_V__DOT__alu_result);
        }

        dut->clk = 1; dut->eval();   // posedge: PC and regfile actually update here

        uint32_t pc_check = dut->rootp->Advaita_V__DOT__pc_address;
        if (pc_check < BASE || pc_check >= BASE + MEM_BYTES) {
            fprintf(stderr, "PC OUT OF RANGE at cycle %llu: PC=0x%08x\n",
                    (unsigned long long)cycle, pc_check);
            break;
        }
